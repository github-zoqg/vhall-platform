<template>
  <div class="subscribe-embed-info__tab">
    ...
    <div v-if="warmup_paas_record_id && showPlayBtn" class="sub-play-btn" @click="play">
      <span class="iconfont iconbofang_icon"></span>
    </div>
      ...
      subOption: {
        handler(val) {
          if (val && val.type) {
            ...
            this.warmup_paas_record_id = val.warmup_paas_record_id;
            this.show = val.show;
            // this.num = val.num
            console.log('>>>>>>1', val);
            if (val.warmup_paas_record_id && val.verify == 0) {
              // 只在免费情况下显示暖场视频

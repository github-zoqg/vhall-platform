  ...
  <div class="c-msg-item">
    ...
    <div :class="['msg-item', msg.type]">
      ...
      <template v-if="['welcome_msg'].includes(msg.type)">
        <div v-if="msg.nickName!=='' && msg.content!==''"
             class="welcome-msg">
      ...
      <template v-if="['reward_pay_ok'].includes(msg.type)">
        <div class="msg-reward gift"
          :class="((Math.random() * 10) > 1) ? 'purpose' : 'red'">
          <p>
            <span class="user-name">{{msg.nickName | filterName}}</span>
            <span class="gift-name">打赏{{msg.content.num}}元,{{msg.content.text_content | filterContext}}</span>
            <img class="award-img"
      ...
      <template v-if="!['welcome_msg','reward_pay_ok','gift_send_success','free_gift_send'].includes(msg.type)">
        <div class="avatar-wrap"
          @click="setPersonStatus($event,msg)">
          <img class="chat-avatar"
            width="26"
            height="26"
            :src="msg.avatar"
            alt />
          <img v-if="msg.client === 'mobile'"
            class="chat-phone"
            width="9"
            height="12"
        ...
        <div class="msg-content">
          <p class="msg-content_name">
            <span class="nickname"
              @click="setPersonStatus($event,msg)">{{msg.nickName}}</span>
            <template v-if="['text','image'].includes(msg.type)">
              <span v-if="msg.roleName && msg.roleName != '2'"
                class="role"
                :class="msg.roleName | roleClassFilter">{{msg.roleName | roleFilter}}</span>
            </template>
          </p>
          <!-- 图文消息 -->
          <template v-if="['text','image'].includes(msg.type)">
            <p v-if="msg.replyMsg && msg.replyMsg.content && msg.replyMsg.content.text_content"
              class="replymsg-content_body"
              v-html="`${msg.replyMsg.nickName || msg.replyMsg.nick_name} : ${msg.replyMsg.content.text_content}`"></p>
            <p v-if="msg.replyMsg && msg.replyMsg.content && msg.replyMsg.content.image_urls"
              class="msg-content_chat-img-wrapper reply-msg"
              :style="msg.replyMsg.content.text_content && 'margin-top:-3px;'">
              <span v-if="!msg.replyMsg.content.text_content"
                style='color:#dddddd;vertical-align:middle;'>{{msg.replyMsg.nickName}}:</span>
              <img @click="previewImg($event,index,msg.replyMsg.content.image_urls)"
            ...
            <p v-if="msg.content.text_content"
              class="msg-content_body"
              v-html="msg.replyMsg && msg.replyMsg.content?`<span style='color:#4DA1FF;'>回复:</span> ${this.msgContent}`:this.msgContent"></p>
            <p v-if="msg.content.image_urls"
              class="msg-content_chat-img-wrapper">
              <span v-if="(msg.replyMsg && msg.replyMsg.content) && !msg.content.text_content"
                style='color:#4DA1FF;vertical-align:middle'>回复:</span>
              <img @click="previewImg($event,index,msg.content.image_urls)"
                class="msg-content_chat-img"
                width="34"
                height="34"
          ...
          <template v-if="msg.type==='lottery_result_notice'">
            <p class="msg-content_body system">
              ...
              <span v-if="msg.content.Show"
                class="lottery-check"
                @click="lotteryCheck($event,msg.content)">点击查看</span>
            </p>
          </template>
          <!-- 推送问卷 -->
          <template v-if="msg.type==='questionnaire_push'">
            <p class="msg-content_body system">
              {{msg.content.text_content}}
              <span v-if="msg.roleName == '2'"
                class="lottery-check"
                @click="questionnaireCheck(msg.content.questionnaire_id)">点击查看</span>
            </p>
          </template>
        </div>
      </template>

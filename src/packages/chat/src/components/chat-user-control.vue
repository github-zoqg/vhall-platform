<template>
  <div class="vhall-chat-user-control_bg"
    v-show="isShow"
    @click.capture="isShow=false">
    <div class="vhall-chat-user-control"
      :style="style">
      <div class="setspeak"
        @click="reply(count)">
        <i></i>
      ...
      <div :class="this.atList.find(u => u.nickName == this.nickName)?'setkick disabled':'setkick'"
        @click="atUser(accountId)">
        <i></i>
        <span>@TA</span>
      </div>
      <div class="setspeak"
        @click="deleteMsg(count)"
        v-if="godMode">
        <i></i>
        <span>删除</span>
      </div>
  ...
  methods: {
    ...
    setBanned() {
      ...
      this.$vhall_paas_port({
        k: 110122,
        data: { business_uid: this.userId, user_id: '', webinar_id: this.$route.params.il_id, refer: '', s: '', report_extra: {}, ref_url: '', req_url: '' }
      });
    },
    /**
     * 踢出/取消踢出

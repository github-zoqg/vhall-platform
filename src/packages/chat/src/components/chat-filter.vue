<template>
  <transition name="chat-filter-slide">
    <div class="chat-filter"
      v-show="chatFilterShow">
      <p v-auth="100017"
        class="chat-banned-all">
        <input class="chat-filter-input"
          v-model="bannedAll"
          @change="handleChange"
          type="checkbox"
          id="disableSendMsg" />
        <label class="chat-filter-label"
          for="disableSendMsg">全员禁言</label>
      </p>
      <a v-auth="100015"
        v-if="!isAssistant"
        ...
        :href="filterUrl"
        target="_blank"
        @click="sendMsgFilter">开启手动过滤</a>
      <a v-auth="100015"
        v-if="isAssistant"
        class="chat-filter-btn"
        @click="sendMsgToAssistant">开启手动过滤</a>
    </div>
  </transition>
</template>

<template>
  <el-dialog
    ...
    :class="phoneData.type == 'edit' && phoneData.step == 1? 'vmp-user-account-phone phone-edit' : 'vmp-user-account-phone'"
    ...
    <div class="vmp-user-account-phone-wrap">
      ...
      <el-form ref="setPhoneForm" :model="setPhoneForm" :rules="setFormRules" label-width="0">
        <p v-if="phoneData.type == 'add'" class="vmp-user-account__sTitle">{{ $t('account.account_1027') }}</p>
        <p v-if="phoneData.type == 'edit' && phoneData.step === 1" class="vmp-user-account__eTitle">{{ $t('account.account_1032') }}</p>
        <p v-if="phoneData.type == 'edit' && phoneData.step === 1" class="vmp-user-account__eNotice">{{ $t('account.account_1033') }}</p>
        <p v-if="phoneData.type == 'edit' && phoneData.step === 2" class="vmp-user-account__eTitle">{{ $t('account.account_1034') }}</p>
        <el-form-item v-if="phoneData.type == 'edit' && phoneData.step === 1" class="vmp-user-account-redonly">
        ...
        <el-form-item prop="code" class="vmp-user-account-wrap__code">
          ...
          <span
            ...
            :class="['vmp-user-account-wrap__code__btn', setPhoneForm.imgCode && !timer ? 'start' : 'disabled']"
  ...
  methods: {
    ...
    getCaptha() {
      ...
      if (validateList.every(item => item === '')) {
        const data = this.phoneData.type === 'edit' ? this.phoneData.phone : this.setPhoneForm.phone;
        ...
        this.useUserServer.sendPhoneCode({
          type: 1,
          scene_id,
          data,
          validate
    ...
    checkOldPhoneHandler(){
      this.$refs.setPhoneForm.validate(valid => {
        if (valid) {
          ...
          this.useUserServer.codeCheck(params).then(async res => {
            if (res && res.code == 200 && res.data.check_result === 1) {
              this.$refs.NECaptcha.refreshNECaptha(); // 重置易盾
              this.$refs.setPhoneForm?.resetFields(); // 重置表单
              this.initInterval(); // 初始化定时器
    ...
    editPhoneHandler() {
      this.$refs.setPhoneForm.validate(valid => {
        if (valid) {
          ...
          this.useUserServer.codeCheck(params).then(res => {
            if (res && res.code == 200 && res.data.check_result === 1) {
              this.bindPhoneSave(res.data.key);
            } else {
              this.$message({

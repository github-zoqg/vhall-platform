...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goWatchUrl() {
        window.location.href = `${window.location.protocol}${process.env.VUE_APP_WAP_WATCH}${process.env.VUE_APP_ROUTER_BASE_URL}/lives/watch/${this.webinarId}${window.location.search}`;
      },
      handleAuthInfo() {
        let data = {
